         ...
         $user_status = mysqli_real_escape_string($conn,$_POST['admin_status']);
			...
			$q_checkadmin = $conn->query("SELECT * FROM `admin_login` WHERE `admin_user` = '$user_email'") or die(mysqli_error());
				...
				if($v_checkadmin == 1){
					...
					$conn->query("INSERT INTO `admin_login` VALUES('','$user_name', '$user_email','$admin_position', '$user_password', '$user_status' )") or die(mysqli_error());
					echo '
						<script type = "text/javascript">
							alert("Successful register admin");
