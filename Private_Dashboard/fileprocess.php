 ...
 /* $link = $_POST['link'];*/
  ...
  $query=mysqli_query($conn,"SELECT * FROM upload_files WHERE name = '$filename'")or die(mysqli_error($conn));
        ...
        if (move_uploaded_file($file, $destination)) {
            $sql = "INSERT INTO upload_files (name, size, download, timers, admin_status, email, location, fileno, month, file_department) VALUES ('$filename', $size, 0, '$time', 'Admin', '$user', '$location_address', '$fileno', '$month', '$file_department')";
