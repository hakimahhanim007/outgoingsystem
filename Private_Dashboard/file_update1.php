...
if (isset($_POST['update'])) { // if save button on the form is clicked
  ...
  $query=mysqli_query($conn,"SELECT * FROM `upload_files` WHERE `name` = '$filename'")or die(mysqli_error($conn));
        ...
        if (move_uploaded_file($file, $destination)) {
            $sql = "UPDATE upload_files (name, size, timers, admin_status, email, location_address, fileno, month, file_department) VALUES ('$filename', $size, '$time', 'Admin', '$user', '$location_address', '$fileno', '$month', '$file_department')";
