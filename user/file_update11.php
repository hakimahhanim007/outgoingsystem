...
if (isset($_POST['update'])) { // if save button on the form is clicked
    // name of the uploaded file
        ...
        // move the uploaded (temporary) file to the specified destination
            ...
            $query = 'UPDATE upload_files set name="'.$filename.'", email= "'.$name_update.'", size = "'.$size.'", month = "'.$month.'", admin_status = "Staff", timers = "'.$timers.'", fileno = "'.$fileno.'",  file_department = "'.$file_department.'",  location_address = "'.$location_address.'" WHERE id ="'.$id.'" ';
